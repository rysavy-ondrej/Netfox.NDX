...
namespace Ndx.Decoders.Basic
{
  public sealed partial class Dns
  {
    ...
    public static Dns DecodeJson(JToken token)
    {
      ...
      {
        var val = token["dns_dns_csync_flags_immediate"];
        if (val != null) obj.DnsCsyncFlagsImmediate = Convert.ToInt32(val.Value<string>(), 10) != 0;
      }
      {
        var val = token["dns_dns_csync_flags_soaminimum"];
      ...
      {
        var val = token["dns_dns_spf_length"];
      ...
      {
        var val = token["dns_dns_ilnp_nid_preference"];
      ...
      {
        var val = token["dns_dns_ilnp_l32_preference"];
      ...
      {
        var val = token["dns_dns_ilnp_l64_preference"];
      ...
      {
        var val = token["dns_dns_ilnp_lp_preference"];
      ...
      }
      {
        ...
        if (val != null) obj.DnsEui48 = val.Value<string>();
      }
      {
        var val = token["dns_dns_eui64"];
        if (val != null) obj.DnsEui64 = default(UInt64);
      }
      {
      ...
      {
        var val = token["dns_dns_opt_padding"];
      ...
      {
        var val = token["dns_dns_nsec3_flags_opt_out"];
      ...
      {
        var val = token["dns_dns_sshfp_fingerprint_type"];
      ...
      {
        var val = token["dns_dns_hip_hit_length"];
      ...
      {
        var val = token["dns_dns_hip_pk_length"];
      ...
      {
        var val = token["dns_dns_caa_flags_issuer_critical"];
