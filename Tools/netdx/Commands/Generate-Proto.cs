...
namespace Netdx
{
    internal class GenerateProto
    {
        ...
        public int Execute(string inpath, string outpath, bool generateCSharp = false)
        {
            ...
            {
                outfile.WriteLine($"// This is file was generated by netdx on ({DateTime.Now}.");
            ...
            if (generateCSharp)
                using (var outfile = new StreamWriter(File.Open(Path.ChangeExtension(outpath, ".Decode.cs"), FileMode.Create, FileAccess.Write)))
                {
                    ...
                    foreach (var field in protocol.FieldMap)
                    {
                        ...
                        outfile.WriteLine($"        if (val != null) {{ var propValue = val.Value<string>(); obj.{fieldName} = {convert}; }}");                        
                        outfile.WriteLine("      }");
                    }
                    outfile.WriteLine("      return obj;");
                    outfile.WriteLine("    }");
                    // Decode (JsonTextReader
                    outfile.WriteLine(
                  $"    public static {protocolName} DecodeJson(JsonTextReader reader)                        \n" +
                   "    {                                                                                     \n" +
                   "        if (reader.TokenType != JsonToken.StartObject) return null;                       \n" +
                  $"        var obj = new {protocolName}();                                                   \n" +
                  @"int openObjects = 0;
                    ...
                    outfile.WriteLine(
                  $"    static void SetField({protocolName} obj, string propName, string propValue)           \n"+
                   "    {                                                                                     \n"+
                   "      switch (propName)                                                                   \n"+
                   "      {                                                                                   "
                    );
                    foreach (var field in protocol.FieldMap)
                    {
                        ...
                        outfile.WriteLine(                                  
                       $"      case \"{tsharkFieldName}\": obj.{fieldName} = {convert}; break;");
        ...
        public static Dictionary<ulong, string> decoders =
            ...
            {                                                              
                { _t(FieldType.FtBoolean,FieldDisplay.BaseNone),   "Convert.ToInt32(propValue, 10) != 0"},
                { _t(FieldType.FtChar,FieldDisplay.BaseDec),       "Convert.ToUInt32(propValue, 10)"},
                { _t(FieldType.FtUint8,FieldDisplay.BaseDec),      "Convert.ToUInt32(propValue, 10)"},
                { _t(FieldType.FtUint16,FieldDisplay.BaseDec),     "Convert.ToUInt32(propValue, 10)"},
                { _t(FieldType.FtUint24,FieldDisplay.BaseDec),     "Convert.ToUInt32(propValue, 10)"},
                { _t(FieldType.FtUint32,FieldDisplay.BaseDec),     "Convert.ToUInt32(propValue, 10)"},
                { _t(FieldType.FtUint40,FieldDisplay.BaseDec),     "Convert.ToUInt64(propValue, 10)"},
                { _t(FieldType.FtUint48,FieldDisplay.BaseDec),     "Convert.ToUInt64(propValue, 10)"},
                { _t(FieldType.FtUint56,FieldDisplay.BaseDec),     "Convert.ToUInt64(propValue, 10)"},
                { _t(FieldType.FtUint64,FieldDisplay.BaseDec),     "Convert.ToUInt64(propValue, 10)"},
                { _t(FieldType.FtInt8,FieldDisplay.BaseDec),       "Convert.ToInt32(propValue, 10)"},
                { _t(FieldType.FtInt16,FieldDisplay.BaseDec),      "Convert.ToInt32(propValue, 10)"},
                { _t(FieldType.FtInt24,FieldDisplay.BaseDec),      "Convert.ToInt32(propValue, 10)"},
                { _t(FieldType.FtInt32,FieldDisplay.BaseDec),      "Convert.ToInt32(propValue, 10)"},
                { _t(FieldType.FtInt40,FieldDisplay.BaseDec),      "Convert.ToInt64(propValue, 10)"},
                { _t(FieldType.FtInt48,FieldDisplay.BaseDec),      "Convert.ToInt64(propValue, 10)"},
                { _t(FieldType.FtInt56,FieldDisplay.BaseDec),      "Convert.ToInt64(propValue, 10)"},
                { _t(FieldType.FtInt64,FieldDisplay.BaseDec),      "Convert.ToInt64(propValue, 10)"},
                { _t(FieldType.FtUint16,FieldDisplay.BasePtUdp),     "Convert.ToUInt32(propValue, 10)"},
                { _t(FieldType.FtChar,FieldDisplay.BaseHex),       "Convert.ToUInt32(propValue, 16)"},
                { _t(FieldType.FtUint8,FieldDisplay.BaseHex),      "Convert.ToUInt32(propValue, 16)"},
                { _t(FieldType.FtUint16,FieldDisplay.BaseHex),     "Convert.ToUInt32(propValue, 16)"},
                { _t(FieldType.FtUint24,FieldDisplay.BaseHex),     "Convert.ToUInt32(propValue, 16)"},
                { _t(FieldType.FtUint32,FieldDisplay.BaseHex),     "Convert.ToUInt32(propValue, 16)"},
                { _t(FieldType.FtUint40,FieldDisplay.BaseHex),     "Convert.ToUInt64(propValue, 16)"},
                { _t(FieldType.FtUint48,FieldDisplay.BaseHex),     "Convert.ToUInt64(propValue, 16)"},
                { _t(FieldType.FtUint56,FieldDisplay.BaseHex),     "Convert.ToUInt64(propValue, 16)"},
                { _t(FieldType.FtUint64,FieldDisplay.BaseHex),     "Convert.ToUInt64(propValue, 16)"},
                { _t(FieldType.FtInt8,FieldDisplay.BaseHex),       "Convert.ToInt32(propValue, 16)"},
                { _t(FieldType.FtInt16,FieldDisplay.BaseHex),      "Convert.ToInt32(propValue, 16)"},
                { _t(FieldType.FtInt24,FieldDisplay.BaseHex),      "Convert.ToInt32(propValue, 16)"},
                { _t(FieldType.FtInt32,FieldDisplay.BaseHex),      "Convert.ToInt32(propValue, 16)"},
                { _t(FieldType.FtInt40,FieldDisplay.BaseHex),      "Convert.ToInt64(propValue, 16)"},
                { _t(FieldType.FtInt48,FieldDisplay.BaseHex),      "Convert.ToInt64(propValue, 16)"},
                { _t(FieldType.FtInt56,FieldDisplay.BaseHex),      "Convert.ToInt64(propValue, 16)"},
                { _t(FieldType.FtInt64,FieldDisplay.BaseHex),      "Convert.ToInt64(propValue, 16)"},

                { _t(FieldType.FtChar,FieldDisplay.BaseOct),       "Convert.ToUInt32(propValue, 8)"},
                { _t(FieldType.FtUint8,FieldDisplay.BaseOct),      "Convert.ToUInt32(propValue, 8)"},
                { _t(FieldType.FtUint16,FieldDisplay.BaseOct),     "Convert.ToUInt32(propValue, 8)"},
                { _t(FieldType.FtUint24,FieldDisplay.BaseOct),     "Convert.ToUInt32(propValue, 8)"},
                { _t(FieldType.FtUint32,FieldDisplay.BaseOct),     "Convert.ToUInt32(propValue, 8)"},
                { _t(FieldType.FtUint40,FieldDisplay.BaseOct),     "Convert.ToUInt64(propValue, 8)"},
                { _t(FieldType.FtUint48,FieldDisplay.BaseOct),     "Convert.ToUInt64(propValue, 8)"},
                { _t(FieldType.FtUint56,FieldDisplay.BaseOct),     "Convert.ToUInt64(propValue, 8)"},
                { _t(FieldType.FtUint64,FieldDisplay.BaseOct),     "Convert.ToUInt64(propValue, 8)"},
                { _t(FieldType.FtInt8,FieldDisplay.BaseOct),       "Convert.ToInt32(propValue, 8)"},
                { _t(FieldType.FtInt16,FieldDisplay.BaseOct),      "Convert.ToInt32(propValue, 8)"},
                { _t(FieldType.FtInt24,FieldDisplay.BaseOct),      "Convert.ToInt32(propValue, 8)"},
                { _t(FieldType.FtInt32,FieldDisplay.BaseOct),      "Convert.ToInt32(propValue, 8)"},
                { _t(FieldType.FtInt40,FieldDisplay.BaseOct),      "Convert.ToInt64(propValue, 8)"},
                { _t(FieldType.FtInt48,FieldDisplay.BaseOct),      "Convert.ToInt64(propValue, 8)"},
                { _t(FieldType.FtInt56,FieldDisplay.BaseOct),      "Convert.ToInt64(propValue, 8)"},
                { _t(FieldType.FtInt64,FieldDisplay.BaseOct),      "Convert.ToInt64(propValue, 8)"},

                { _t(FieldType.FtFloat,FieldDisplay.BaseNone),     "Convert.ToSingle(propValue)"},
                { _t(FieldType.FtDouble,FieldDisplay.BaseNone),    "Convert.ToDouble(propValue)"},
                { _t(FieldType.FtRelativeTime,FieldDisplay.BaseNone), "Convert.ToSingle(propValue)"},

                { _t(FieldType.FtString,FieldDisplay.BaseNone),    "propValue"},
                { _t(FieldType.FtBytes,FieldDisplay.BaseNone),     "StringToBytes(propValue)"},

                { _t(FieldType.FtEther,FieldDisplay.BaseNone),     "Google.Protobuf.ByteString.CopyFrom(System.Net.NetworkInformation.PhysicalAddress.Parse(propValue.ToUpperInvariant().Replace(':','-')).GetAddressBytes())"},
                { _t(FieldType.FtIpv4,FieldDisplay.BaseNone),      "Google.Protobuf.ByteString.CopyFrom(System.Net.IPAddress.Parse(propValue).GetAddressBytes())"},
                { _t(FieldType.FtIpv6,FieldDisplay.BaseNone),      "Google.Protobuf.ByteString.CopyFrom(System.Net.IPAddress.Parse(propValue).GetAddressBytes())"},
            };
        ...
        private PbTypes GetPbType(FieldType type)
        {
            switch(type)
            {
                ...
                case FieldType.FtRelativeTime: return PbTypes.Float;
