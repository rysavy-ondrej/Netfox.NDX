...
{
  public Rule_comment(String spelling, ArrayList<Rule> rules)
  {
  ...
  public static Rule_comment parse(ParserContext context)
  {
        ...
        while (f1)
        {
          ...
          parsed = false;
          {
            ...
            if (parsed)
            {
              ...
              {
                Rule rule = Rule_ctext.parse(context);
                if ((f2 = rule != null))
                {
    ...
    if (parsed)
    {
        rule = new Rule_comment(context.text.substring(a0.start, a0.end), a0.rules);
    }
    else
    {
        context.index = s0;
